      ...
      <nav aria-label="Page navigation">
        <ul className="inline-flex">
          ...
          {pageLinks.map(page => (
            <li key={page}>
              <button
                className={`px-4 py-2 ${currentPage === page ? 'text-white bg-green-600' : 'text-green-600 bg-white'} transition-colors duration-150 border border-r-0 border-green-600 focus:shadow-outline ${
                  currentPage === page ? 'rounded-l-none rounded-r-none' : ''
                }`}
                onClick={() => {
                  handleGoToPage(page);
                  scrollToTop();
